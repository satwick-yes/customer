...
/**
 * Builds a jsPDF document for a booking
 */
function buildJobSheetDoc(booking) {
...
/**
 * Saves a job sheet PDF to a file path on local filesystem.
 */
function saveJobSheetPdfToFile(booking, outputDir) {
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }

  const doc = buildJobSheetDoc(booking);
  if (!doc) return null;

  const fileName = `JobSheet_${booking.jobId || 'Booking'}.pdf`;
